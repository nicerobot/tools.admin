...
```
ghcr.io/nicerobot/tools.admin/go-tooling:v3
```
...
jobs:
  go-quality:
    runs-on: ubuntu-latest
    container: ghcr.io/nicerobot/tools.admin/go-tooling:v3
    steps:
      - uses: actions/checkout@v4
...
- **[`Makefile`](../examples/go-repo/Makefile)** is for laptops with no
  ...
  ```makefile
  GO_TOOLING_IMAGE ?= ghcr.io/nicerobot/tools.admin/go-tooling:v3
  %:
...
```makefile
GO_TOOLING_IMAGE = ghcr.io/nicerobot/tools.admin/go-tooling@sha256:<digest>
```
...
`v*` tags as `ghcr.io/nicerobot/tools.admin/go-tooling:<tag>` and `:latest`.
