...
class TestCleanupRuns:
    ...
    def test_auto_detects_current_repo(
        self, monkeypatch: pytest.MonkeyPatch,
    ) -> None:
        monkeypatch.setenv("GITHUB_REPOSITORY", "acme/widget")
        runs = [_run(1)]
        mock = _mock_client(runs_by_repo={"widget": runs})
        with patch(
            "admin_tools.commands.cleanup_runs.GitHubClient",
            return_value=mock,
        ):
            run_cleanup_runs(None, None, days=30, keep=0, dry_run=False)
    ...
    def test_missing_owner_without_env_raises(
        self, monkeypatch: pytest.MonkeyPatch,
    ) -> None:
        ...
        with patch(
            "admin_tools.commands.cleanup_runs.GitHubClient",
            return_value=mock,
        ), pytest.raises(RuntimeError, match="GITHUB_REPOSITORY"):
            run_cleanup_runs(None, None, days=30, keep=0, dry_run=False)
