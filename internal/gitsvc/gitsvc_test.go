...
type recorder struct {
	calls   [][]string
	results []gitsvc.Result
	errs    []error
	i       int
}

func (r *recorder) run(args []string) (gitsvc.Result, error) {
...
func newService(results []gitsvc.Result, errs []error) (gitsvc.Service, *recorder) {
	rec := &recorder{results: results, errs: errs}
	return gitsvc.New(rec.run), rec
}
...
func TestConfigureBotIdentityFirstFails(t *testing.T) {
	...
	require.ErrorIs(t, svc.ConfigureBotIdentity(), adminerr.ErrCommand)
...
func TestCheckedNonZeroExit(t *testing.T) {
	svc, _ := newService([]gitsvc.Result{{ExitCode: 1}}, nil)
	require.ErrorIs(t, svc.CheckoutBranch("b"), adminerr.ErrCommand)
}
...
func TestHasStagedChangesFalse(t *testing.T) {
	svc, _ := newService([]gitsvc.Result{{ExitCode: 0}}, nil)
	has, err := svc.HasStagedChanges()
	require.NoError(t, err)
	assert.False(t, has)
}
...
func TestPrExistsTrue(t *testing.T) {
	svc, rec := newService([]gitsvc.Result{{Stdout: "42\n"}}, nil)
...
func TestPrExistsFalse(t *testing.T) {
	svc, _ := newService([]gitsvc.Result{{Stdout: ""}}, nil)
	exists, err := svc.PrExists("my-branch")
	require.NoError(t, err)
