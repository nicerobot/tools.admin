...
- **Go Tooling Image** - Prebuilt container bundling a full Go quality toolchain so other repos consume it via CI without embedding any tooling
  ...
  - Consumer template in [`examples/go-repo/`](examples/go-repo/) (project `Makefile` + workflow) plus action-based [`examples/go-quality.yml`](examples/go-quality.yml)
  - Local development uses the same pinned image as CI (same tool versions, nothing installed on the host): the example Makefile includes `tools.mk` when inside the image and otherwise proxies each target into the image via `docker run`
