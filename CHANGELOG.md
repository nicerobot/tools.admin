...
- **Go Tooling Image** - Prebuilt container bundling a full Go quality toolchain so other repos consume it via CI without embedding any tooling
  ...
  - Built on the latest official `golang` image (Go 1.26; tools require Go >= 1.25); published by `release.yml` as `ghcr.io/nicerobot/admin-tools/go-tooling:<tag>` and `:latest`; CI builds the image and smoke-tests the gate against a sample module
  - Consumer template in [`examples/go-repo/`](examples/go-repo/): `ci.mk` (runs inside the image, `include`s `tools.mk`) + a local-dev `Makefile` (proxies each target into the pinned image via `docker run`) + workflow; plus action-based [`examples/go-quality.yml`](examples/go-quality.yml)
  - Local development uses the same pinned image as CI, so tool versions match with nothing installed on the host
  - revive is configured in the shipped `.golangci.yml` (YAML) and runs via golangci-lint; no separate revive TOML config is shipped
