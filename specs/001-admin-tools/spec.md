...
**Independent Test**: Run `admin-tools snapshot --owner nicerobot` against a real or mocked GitHub API and verify the output YAML files match expected overrides.

**Acceptance Scenarios**:

1. **Given** an org with 5 repos and a settings.yml defining defaults, **When** `admin-tools snapshot --owner myorg` runs, **Then** 5 YAML files are written to `.github/repos/` with correct overrides.
...
**Independent Test**: Run `admin-tools create-pr` after staging changes and verify a PR is created (or skipped if no changes exist).

**Acceptance Scenarios**:

1. **Given** staged changes in `.github/repos/`, **When** `admin-tools create-pr` runs, **Then** a commit is created, the branch is force-pushed, and a PR is opened.
...
Other org admin repos consume admin-tools as `uses: nicerobot/admin-tools@v1` in their workflows, replacing shell scripts and `yq` dependencies.

**Why this priority**: The packaging as a reusable action is the deployment mechanism.

**Independent Test**: Create a workflow in a test repo using `nicerobot/admin-tools@v1` with both `snapshot` and `create-pr` commands and verify it runs successfully.

**Acceptance Scenarios**:

1. **Given** a workflow with `command: snapshot` and `owner: myorg`, **When** the action runs, **Then** `entrypoint.sh` routes to `admin-tools snapshot --owner myorg`.
2. **Given** a workflow with `command: create-pr`, **When** the action runs, **Then** `entrypoint.sh` routes to `admin-tools create-pr` with the specified branch and base.
...
**Independent Test**: Run `admin-tools cleanup-runs --owner nicerobot --dry-run` and verify it lists runs that would be deleted without actually deleting them.
